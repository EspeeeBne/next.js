...
function pingRootRouteTree(
  ...
  switch (route.status) {
    ...
    case EntryStatus.Fulfilled: {
      ...
      // We don't need to do this for dynamic prefetches, because those are only available in
...
function pingRouteTreeAndIncludeDynamicData(
  ...
  switch (segment.status) {
    ...
    case EntryStatus.Fulfilled: {
      ...
      if (
        segment.isPartial &&
        isFetchStrategyLessSpecific(segment.fetchStrategy, fetchStrategy)
      ) {
        // The cached segment contains dynamic holes, and was prefetched using a less specific strategy than the current one.
        // This means we're in one of these cases:
        //   - we have a static prefetch, and we're doing a dynamic prefetch
        //   - we have a static or dynamic prefetch, and we're doing a Full prefetch (or a navigation).
    ...
    case EntryStatus.Rejected: {
      ...
      // attempt failed. If it used a less specific strategy than the current one, fetch it again.
      if (isFetchStrategyLessSpecific(segment.fetchStrategy, fetchStrategy)) {
        spawnedSegment = pingFullSegmentRevalidation(
          now,
          task,
          route,
