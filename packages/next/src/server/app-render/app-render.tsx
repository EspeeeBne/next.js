...
function parseRequestHeaders(
  ...
  const isPrefetchRequest =
    isDevWarmupRequest || headers[NEXT_ROUTER_PREFETCH_HEADER_LOWERCASE] === '1'
...
async function generateDynamicPrefetchResult(
  ...
  function onFlightDataRenderError(err: DigestedError) {
    return renderOpts.onInstrumentationRequestError?.(
      err,
      req,
      // TODO(dynamic-ppr): should we use a different value?
      createErrorContext(ctx, 'react-server-components-payload')
    )
  }
  const onError = createFlightReactServerErrorHandler(
  ...
  // TODO: create a a prerenderResumeDataCache here and pass it to both
  ...
  await prospectiveDynamicServerPrerender(
    ctx,
    generatePayload,
  ...
  const response = await finalDynamicServerPrerender(
    ctx,
    generatePayload,
    prerenderResumeDataCache,
    renderResumeDataCache,
...
async function prospectiveDynamicServerPrerender(
  ctx: AppRenderContext,
  getPayload: () => any,
  prerenderResumeDataCache: PrerenderResumeDataCache | null,
  renderResumeDataCache: RenderResumeDataCache | null,
  rootParams: Params,
  cookies: PrerenderStoreModernDynamic['cookies'],
  draftMode: PrerenderStoreModernDynamic['draftMode']
) {
  ...
  const initialServerPrerenderStore: PrerenderStoreModernDynamic = {
    ...
    // A dynamic prerender has all the params available.
    fallbackRouteParams: null,
    // These are not present in regular prerenders, but allowed in a dynamic prerender.
    cookies,
    draftMode,
  }
...
async function finalDynamicServerPrerender(
  ctx: AppRenderContext,
  getPayload: () => any,
  prerenderResumeDataCache: PrerenderResumeDataCache | null,
  renderResumeDataCache: RenderResumeDataCache | null,
  ...
  const finalServerPrerenderStore: PrerenderStoreModernDynamic = {
    type: 'prerender-dynamic',
    phase: 'render',
    rootParams,
    implicitTags,
    ...
    async () => {
      const prerenderResult = await workUnitAsyncStorage.run(
        ...
        clientReferenceManifest.clientModules,
        {
          filterStackFrame,
          onError: (err) => {
            // TODO: tell the client router if a dynamic prerender errored so it can retry a full dynamic request?
            return onError(err)
          },
